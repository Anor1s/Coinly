...
const StackedBarChartCreate = {
  ...
  getChartOptions() {
    ...
    return {
      ...
      onResize: (chart, size) => {
        const months = this.getMonthsToShow(size.width);
        if (months !== this.monthsToShow) {
  ...
  async updateChart(forceRefresh = false) {
    await ChartsTemplate.fetchAndRefresh(this, forceRefresh, (rawData) => {
      ...
      const displayData = stats.slice(0, this.monthsToShow);
  ...
  async init(chartId) {
    ...
    this.monthsToShow = this.getMonthsToShow(container?.clientWidth || 0);
